...
class VeEditor(cmd.Cmd):
    ...
    def do_move(self, arg):
        ...
        if direction == 'start':
            ...
            current_line = self.state['content'][self.state['cursor']['row']] if self.state['content'] else ''
    ...
    def do_line(self, arg):
        ...
        if arg.lower() == 'start':
            ...
            # 调整列位置到有效范围内
            line_length = len(self.state['content'][0]) if self.state['content'] else 0
            ...
            # 调整列位置到有效范围内
            line_length = len(self.state['content'][self.state['cursor']['row']]) if self.state['content'] else 0
            self.state['cursor']['col'] = min(self.state['cursor']['col'], line_length)
            self.render_editor()
            return
