...
class VeEditor(cmd.Cmd):
    ...
    def do_move(self, arg):
        """移动光标: 
        move left/right [数量]（默认1格）或 mv l/r [数量]
        move start - 移动到当前行首 或 mv st
        move end - 移动到当前行尾 或 mv ed"""
    ...
    def do_line(self, arg):
        """将光标移动到指定行: 
        line <行号> - 将光标移动到指定行 或 ln <行号>
        line start - 将光标移动到第一行 或 ln st
        line end - 将光标移动到最后一行 或 ln ed"""
        if not self.state['is_active']:
            print("请先使用 open <文件名> 或 op <文件名> 打开一个文件")
            return
            
        current_line = len(self.state['content'])
        if current_line == 0:
            # 确保内容不为空
            self.state['content'] = ['']
            current_line = 1
    ...
    def do_del(self, arg):
        ...
        del range all - 删除全部内容 或 del rng a
        del range <开始> <结束> - 删除指定范围内容 或 del rng <开始> <结束>"""
        if not self.state['is_active']:
            print("请先使用 open <文件名> 或 op <文件名> 打开一个文件")
            return
    ...
    def do_copy(self, arg):
        """复制操作:
        copy - 复制全部内容 或 cp
        copy range <开始> <结束> - 复制指定范围内容 或 cp rng <开始> <结束>"""
        if not self.state['is_active']:
            print("请先使用 open <文件名> 或 op <文件名> 打开一个文件")
            return
    ...
    def do_paste(self, arg):
        """粘贴操作:
        paste - 在光标前粘贴内容 或 pt
        paste range <开始> <结束> - 剪切指定范围并粘贴 或 pt rng <开始> <结束>"""
        if not self.state['is_active']:
            print("请先使用 open <文件名> 或 op <文件名> 打开一个文件")
            return
